...
;(function($, window, document, undefined) {
	...
	var Utils = {
		 ...
		 * @memberOf Utils
		 * @param {string}
		 *            name The name of the LESS class.
		 * @param {string}
		 *            prop The css property where the data is stored.
		...
		var
			...
			$tabs = (function() {
				...
				$contents.each(function() {
					...
					var html = '<li class="ideal-tabs-tab">' +
						'<span>' + name + '</span>' +
	...
	$.fn.idealSelect = function() {
		...
		return this.each(function() {
			...
			var idealSelect = (function() {
				var $wrap = $('<ul class="ideal-select ' + $select.attr('') + '"/>'),
					$menu = $(
			...
			var Actions = {
				...
				scrollIntoView : function(dir) {
					var $selected = idealSelect.items.filter('.ideal-select-item-selected'),
								...
								$matches = idealSelect.items
									.filter(function() {
										return /^\w+$/i.test(letter) && // not
																		...
																		// super...
																		// )
											new RegExp('^' + letter, 'i').test($(this).text()) // find
																								// first
																								// match
									}),
			...
			/**
			 * @namespace Holds all events of custom select for "menu mode" and
			 *            "list mode"
			 * @memberOf $.fn.toCustomSelect
			 */
			var events = {
	...
	/*
	 * idealRadioCheck: jQuery plguin for checkbox and radio replacement Usage:
	...
	;(function($) {
		...
		$.fn.idealFile = function() {
			...
			return this.each(function() {
				...
				var $file = $(this).addClass('ideal-file'), // the original file
															// input
				...
				$file.change(function() {
					...
					$input.val(filename) // Set the value
						.attr('title', filename) // Show filename in title
													// tootlip
				...
				$input.on({
					...
					keydown : function(e) {
						if (e.which === 13) { // Enter
							...
							// a clean clone with all the original events
							// attached
	...
	$.idealforms.errors = {
		...
		phone : '必须是有效的电话号码(以13,14,15,17,18开头且为11位)',
		zip : 'Must be a valid US zip code. <em>(e.g. 33245 or 33245-0003)</em>',
		url : '必须是有效的url. <em>(e.g. www.google.com)</em>',
		minChar : '不能少于<strong>{0}</strong>个字符.',
		maxChar : '不能多于<strong>{0}</strong>个字符.',
	...
	var getFilters = function() {
		...
		var filters = {
			...
			date : {
				regex : function(input, value) {
					var userFormat = input.userOptions.data && input.userOptions.data.date
			...
			dob : {
				regex : function(input, value) {
					var userFormat = input.userOptions.data && input.userOptions.data.dob
...
// self.focusFirstInvalid()
		...
		_validate : function($input, e) {
			...
			// Prevent validation when typing but not introducing any new
			// characters
			...
			if (userFilters) {
				...
				$.each(userFilters, function(i, filter) {
					...
					if (theFilter) {
						...
						// AJAX
						if (filter === 'ajax') {
							showOrHideError(error, false) // set invalid till
															// response comes
															// back
							$error.hide()
							if (e.type === 'keyup') {
								theFilter.regex(inputData, value, showOrHideError) // runs
																					// the
																					// ajax
																					// callback
							} else {
						...
						// All other filters
							...
							error = customError || theFilter.error // assign
																	// error
																	// after
																	// calling
																	// regex()
