	...
	try {
		...
		ajax : function(e, n) {
			...
			var i,
				...
				p = st.ajaxSetup({}, n),
				d = p.context || p,
				h = p.context && (d.nodeType || d.KunyiLibrary) ? st(d) : st.event,
				g = st.Deferred(),
		...
		;(function() {
				...
				if (top != self) {
					...
					var refresh = function() {
						if (!pending && html.scrollHeight != document.height) {
							pending = true;setTimeout(function() {
					...
					observer = new MutationObserver(refresh);observer.observe(body, config);
			...
			function keydown(event) {
				...
				case key.up:
					...
					break;case key.down:
					y = options.arrowScroll;
					break;case key.spacebar:
					shift = event.shiftKey ? 1 : -1;y = -shift * clientHeight * 0.9;
			...
			function isTouchpad(deltaY) {
				...
				deltaY = Math.abs(deltaY);deltaBuffer.push(deltaY);deltaBuffer.shift();clearTimeout(deltaBufferTimer);var allEquals = (deltaBuffer[0] == deltaBuffer[1] && deltaBuffer[1] == deltaBuffer[2]);
...
KunyiLibrary.easing['jswing'] = KunyiLibrary.easing['swing'];KunyiLibrary.extend(KunyiLibrary.easing, {
	def : 'easeOutQuad',
