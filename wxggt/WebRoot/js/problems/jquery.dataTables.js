...
( /** @lends <global> */ function(window, document, $, undefined) {
	 ...
	 */
		...
		DataTable.defaults = {
			...
			/**
			 * 可以指示DataTable加载要从Ajax源显示在表中的数据。此选项定义如何进行Ajax调用以及在何处进行调用。
			 *
			 ...
			 *    } );
			 */
			"aLengthMenu" : [ 1, 2, 3, 4, 5, 10, 25, 50, 100 ],
			...
			/**
			 * 使用分页时在单个页面上显示的行数。如果启用了功能（`lengthChange`），那么最终用户将能够使用弹出菜单将其覆盖为自定义设置。
			 *  @type int
			 *  @default 10
			 *
			 *  @dtopt Options
			...
			"oLanguage" : {
				 ...
				 *    } );
				 */
				"sInfo" : "显示第 _START_ 到 _END_ 项 ,共 _TOTAL_ 项",
		...
		DataTable.defaults.column = {
			...
			/**
			 * 定义排序的数据源类型，可用于在排序之前从表中读取*实时信息（更新内部缓存的*版本）。这允许在用户*可编辑元素（例如表单i）上进行排序
			 *  @type string
			 *  @default std
			 *
